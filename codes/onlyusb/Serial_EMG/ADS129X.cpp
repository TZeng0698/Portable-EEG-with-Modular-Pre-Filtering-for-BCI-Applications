...
void ADS129X::begin() {
    ...
    SPI2.begin(ADS129X_SCK_PIN, ADS129X_MISO_PIN, ADS129X_MOSI_PIN, ADS129X_SSEL1_PIN);
    SPI2.beginTransaction(SPISettings(10000000, MSBFIRST, SPI_MODE1));

    delay(200); // let power & clocks settle

    // Step 1: Hardware reset pulse
    digitalWrite(ADS129X_RESET_PIN, LOW);
    delayMicroseconds(5);                 // > 2 µs
    digitalWrite(ADS129X_RESET_PIN, HIGH);
    delayMicroseconds(10);                // allow recovery

    // Step 2: RESET command
    RESET(ADS_1);

    // Step 3: Exit RDATAC (must be in SDATAC before config writes)
    SDATAC(ADS_1);
...
void ADS129X::WAKEUP(uint8_t ads) {
    ...
    delayMicroseconds(2);
    csHigh(ads);
    delayMicroseconds(2);  //must wait at least 4 tCLK cycles before sending another command (Datasheet, pg. 38)
    SPI2.endTransaction();
}
